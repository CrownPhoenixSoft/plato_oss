...
import { ListChecks } from "@plato/ui/icons";
...
const components = {
  h2: (props: any) => (
    <h2
      className="text-2xl underline-offset-4 before:absolute before:left-0 before:hidden before:-translate-x-full before:content-['🔗'] hover:underline hover:before:block"
      {...props}
    />
  ),
  a: (props: any) => (
...
export function MDXRemote({ code, images, className }: MDXProps) {
  const MDXImage = (props: any) => {
    // if (!images) return null;
    // const blurDataURL = images.find((image) => image.src === props.src)
    //   ?.blurDataURL;

    return <Image {...props} className="rounded-lg border" />;
  };
